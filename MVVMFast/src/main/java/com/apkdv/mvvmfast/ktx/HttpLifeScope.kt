 ...
 * 此类用于开启协程，并自动捕获异常
 *
 * 在FragmentActivity、 ViewModel环境下，使用 [HttpLifeScope.launch] 方式开启协程，会在页面销毁时，自动关闭协程  (注意：这里的rxLifeScope是变量，不是类名)
 *
 * 其它环境下，需要拿到 [HttpLifeScope.launch]方法的返回值后，手动调用[Job.cancel]方法关闭协程
 * User: ljx
 * Date: 2020-03-14
 * Time: 10:30
 */
class HttpLifeScope() : Closeable {
