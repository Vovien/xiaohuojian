...
public class GridSpaceItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (mStartFromSize <= position && position <= lastPosition - mEndFromSize) {
            ...
            if (mIncludeEdge) {
                inCludeEdge(outRect, position, spanGroupIndex, column, fullSpan);
            } else {
                /*
                 *示例：
    ...
    private void inCludeEdge(@NonNull Rect outRect, int position, int spanGroupIndex, int column, boolean fullSpan) {
        /*
         *示例：
         * spacing = 10 ；spanCount = 3
         * ---------10--------
    ...
    private void unIncludeEdge(@NonNull Rect outRect, int position, int spanGroupIndex, int column, boolean fullSpan) {
        if (fullSpan) {
            outRect.left = 0;
            outRect.right = 0;
        } else {
