...
public class UncaughtExceptionHandlerImpl implements UncaughtExceptionHandler {
     ...
     */
    public void init(Context context, boolean isRestartApp, long restartTime, Class restartActivity) {
        ...
        if (!handleException(ex) && mDefaultHandler != null) {
            ...
            if (!BuildConfig.DEBUG){
                if (mIsRestartApp) { // 如果需要重启
                    Intent intent = new Intent(mContext, mRestartActivity);
    ...
    private String getTips(Throwable ex) {
        if (ex instanceof SecurityException) {
            if (Objects.requireNonNull(ex.getMessage()).contains("android.permission.CAMERA")) {
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            String path = new File(Utils.getApp().getExternalFilesDir(null),  File.separator+"exception/").getPath();
