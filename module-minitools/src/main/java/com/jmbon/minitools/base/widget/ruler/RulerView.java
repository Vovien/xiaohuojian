...
public class RulerView extends View implements ScrollChange {
    ...
    private float scrollVelocity = 0.4f;
    ...
    public RulerView(Context context, int mLineSpace, int mLineWidth, int mSmallLineHeight, int mLongLineHeight, int mSmallSpaceNum, int mLineColor, int mTextColor, boolean mIsFloat, int mStartLineIndex) {
        super(context);
        this.mLineSpace = mLineSpace;
        this.mLineWidth = mLineWidth;
        this.mSmallLineHeight = mSmallLineHeight;
        this.mLongLineHeight = mLongLineHeight;
    ...
    private void init(Context context) {
        initDistanceForDp();
        mRulerHelper = new RulerHelper(this, mSmallSpaceNum, mIsFloat,mStartLineIndex);
    ...
    private void initDistanceForDp() {
        // mLineWidth = dp2px(1);
        // mLineSpace = dp2px(16);
        // mSmallLineHeight = dp2px(16);
        // mLongLineHeight = dp2px(24);
        dpFor05 = dp2px(0.5f);
        dpFor14 = dp2px(14);
    }
    ...
    private void drawRuler(Canvas canvas) {
        ...
        for (int index = 0; index <= mRulerHelper.getCounts(); index++) {
            ...
            if (longLine) {
                //绘制刻度文本
                String text = mRulerHelper.getTextByIndex(mStartLineIndex>0?(mTextIndex+mStartLineIndex):mTextIndex);
                Paint.FontMetrics fontMetrics = mTextPaint.getFontMetrics(); // 获取标题文字的高度（fontMetrics.descent - fontMetrics.ascent）
                float textH = fontMetrics.descent - fontMetrics.ascent;
                mTextIndex += mSmallSpaceNum;
                // canvas.drawText(text, mRect.centerX(), getMeasuredHeight() - dpFor14, mTextPaint);
                if (mIsFloat) {
                    canvas.drawText(((int) Float.parseFloat(text)) + "", mRect.centerX(), getMeasuredHeight() - textH / 2, mTextPaint);
                } else {
                    canvas.drawText(text, mRect.centerX(), getMeasuredHeight() - textH / 2, mTextPaint);
                }
            }
            //绘制刻度
    ...
    private int getStartY(boolean isLong) {
        ...
        return getMeasuredHeight() - dp2px(50);
...
//        }
    ...
    private int getEndY(boolean isLong) {
        ...
        if (isLong) {
            return dp2px(30f) + mLongLineHeight;
            //   return getMeasuredHeight() + (mLongLineHeight - mSmallLineHeight) - dpFor05;
        } else {
            return dp2px(30f) + mSmallLineHeight;
            //  return getMeasuredHeight() - dpFor05;
        }
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (Math.abs(xVelocity) >= mMinVelocity) {
                    ...
                    //滚动速率
                    int velocityX = (int) (xVelocity * scrollVelocity);
    ...
    public void scrollFinish() {
        ...
        if (0 != scrollDistance) {
            ...
            invalidate();
            if (scrollSelected != null && lastText != getCurrentText()) {
