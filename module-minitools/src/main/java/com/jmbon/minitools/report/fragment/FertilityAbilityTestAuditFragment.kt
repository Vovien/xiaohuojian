...
import com.jmbon.minitools.databinding.FragmentAuditFertilityAbilityTestLayoutBinding
...
class FertilityAbilityTestAuditFragment: ViewModelFragment<FertilityAbilityTestViewModel, FragmentAuditFertilityAbilityTestLayoutBinding>() {

    override fun initView(view: View) {
        binding.apply {
            ShadowDrawable.setShadowDrawable(vShadow, Color.TRANSPARENT, 36.dp, ColorUtils.setAlphaComponent(Color.BLACK, 0.15f), 18.dp, 0, 18.dp)
...
//                viewModel.getTestResultPicture(id)
    ...
    private fun savePicture(url: String?) {
        ...
        GlideUtil.getInstance().loadUrlSimpleTarget(url, object :
            ...
            override fun onResourceReady(resource: Drawable?) {
                ...
                context?.let { it1 ->
                    val result = BitmapUtils.saveBitmap(it1, bitmap, "${it1.cacheDir.path}/${System.currentTimeMillis()}.jpeg")
                    if (result) {
                        "已自动保存测试表单信息\n可在本地相册查看".showToast()
                    }
                }
    ...
    companion object {
        ...
        fun newInstance(data: Parcelable?): Fragment {
            return FertilityAbilityTestAuditFragment().apply {
