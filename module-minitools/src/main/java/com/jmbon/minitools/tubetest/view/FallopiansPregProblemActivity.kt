...
@Route(path = TubeArouterConstant.TUBE_SLG_PREG_PROBLEM, name = "输卵管问题页面")
class FallopiansPregProblemActivity :
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        TubeConstant.problemList.add(171)
    }

    override fun initView(savedInstanceState: Bundle?) {
        TubeConstant.problemList.removeAt(0)
        binding.tvTitle.text = getString(R.string.fallopians_problem)
        ...
        SpanUtils.with(binding.tvGuide).append("可以通过").append("腹腔镜、B超、输卵管造影")
            .setForegroundColor(resources.getColor(R.color.color_313EF0)).setBold().append("等检查来了解女方的输卵管能力哦。").create()

        problemItemAdapter.setNewInstance(TubeConstant.baseInfoBean.fallopians)
        ...
        problemItemAdapter.setOnItemClickListener { adapter, view, pos ->
            ...
            if (problemItemAdapter.selectedIds.contains(TubeConstant.baseInfoBean.fallopians[TubeConstant.baseInfoBean.fallopians.size - 1].id)) {
                //其他原因
            ...
            if (problemItemAdapter.selectedIds.contains(TubeConstant.baseInfoBean.fallopians[TubeConstant.baseInfoBean.fallopians.size - 2].id)) {
                //我不清楚
    ...
    private fun initListener() {
        ...
        binding.btnSure.setOnClickListener {
            ...
            TubeConstant.userInfoBean.fallopians = selectedId
            TubeConstant.userInfoBean.other_fallopians = binding.edInput.text.toString()
            //其他次数页面
            ArouterUtil.toProblemActivity(-1)
