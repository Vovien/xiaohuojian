package com.jmbon.video.widget.seekbar;
...
//        initTickXAndTextXLocation();
    ...
    public void setRightSideTrackSize(int size) {
        size = SizeUtils.dp2px(getContext(), size);
        if (mR2L) {
            mProgressTrackSize = size;
        }
        mBackgroundTrackSize = size;
        invalidate();
    }
    ...
    public void setLeftSideTrackSize(int size) {
        size = SizeUtils.dp2px(getContext(), size);
        if (mR2L) {
            mBackgroundTrackSize = size;
        }
    ...
    public void setTrackColor(int backgroundColor, int progressColor) {
        mBackgroundTrackColor = backgroundColor;
        mProgressTrackColor = progressColor;
        invalidate();
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isTouchSeekBar(mX, event.getY())) {
                    ...
                    mIsTouching = true;
                    touchProgress = (calculateTouchX(adjustTouchX(event)) - mPaddingLeft) - (mProgressTrack.right - mProgressTrack.left);
                    return true;
                }
                break;
    ...
    private void refreshSeekBar(MotionEvent event) {
        float current = calculateTouchX(adjustTouchX(event)) - touchProgress;

        refreshThumbCenterXByProgress(getLengthInRange(calculateProgress(current)));
        setSeekListener(true);
        invalidate();
    }

    private float getLengthInRange(float realProgress) {
        realProgress = Math.max(mMin, realProgress);
        realProgress = Math.min(mMax, realProgress);
        return realProgress;
    }
    ...
    private float calculateProgress(float touchX) {
        lastProgress = mProgress;
        float current = getProgressByTouchX(touchX);
        if (rangeInDefined(current, mMin, mMax)) {
            mProgress = current;
        } else {
            mProgress = getLengthInRange(mProgress);
        }
        return mProgress;
    }
    ...
    private float getProgressByTouchX(float touchX) {
        return mMin + (mMax - mMin) * (touchX - mPaddingLeft) / mSeekLength;
    }
