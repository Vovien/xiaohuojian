 ...
 * @Date 2020/3/13
 */
public class CanScrollConsecutiveLayout extends ViewGroup implements ScrollingView, NestedScrollingParent2, NestedScrollingChild2 {
    ...
    private final HashMap<Integer, Float> mFixedYMap = new HashMap<>();
    ...
    public CanScrollConsecutiveLayout(Context context) {
        this(context, null);
    }

    public CanScrollConsecutiveLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CanScrollConsecutiveLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            disableChildHorizontalScroll = a.getBoolean(R.styleable.ConsecutiveScrollerLayout_stickyOffset, false);
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        if (CanScrollUtils.isConsecutiveScrollerChild(child)) {
            View scrollChild = CanScrollUtils.getScrollChild(child);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mDownLocation[0] = CanScrollUtils.getRawX(this, ev, actionIndex);
                mDownLocation[1] = CanScrollUtils.getRawY(this, ev, actionIndex);
                isTouchNotTriggerScrollStick = CanScrollUtils.isTouchNotTriggerScrollStick(this, mDownLocation[0], mDownLocation[1]);
                break;
            ...
            case MotionEvent.ACTION_POINTER_UP:
                ...
                if (mActivePointerId == ev.getPointerId(actionIndex)) { // 如果松开的是活动手指, 让还停留在屏幕上的最后一根手指作为活动手指
                    ...
                    mDownLocation[0] = CanScrollUtils.getRawX(this, ev, newPointerIndex);
                    mDownLocation[1] = CanScrollUtils.getRawY(this, ev, newPointerIndex);
                    isTouchNotTriggerScrollStick = CanScrollUtils.isTouchNotTriggerScrollStick(this, mDownLocation[0], mDownLocation[1]);
                }
                initAdjustVelocityTrackerIfNotExists();
                mAdjustVelocityTracker.addMovement(vtev);
            ...
            case MotionEvent.ACTION_UP:
                if (mAdjustVelocityTracker != null) {
                    ...
                    int touchX = CanScrollUtils.getRawX(this, ev, actionIndex);
                    int touchY = CanScrollUtils.getRawY(this, ev, actionIndex);
                    View targetView = getTouchTarget(touchX, touchY);
                    boolean canScrollVerticallyChild = CanScrollUtils.canScrollVertically(targetView);
                    boolean canScrollHorizontallyChild = CanScrollUtils.isHorizontalScroll(this, touchX, touchY);
                    ...
                    if (SCROLL_ORIENTATION != SCROLL_VERTICAL && !CanScrollUtils.isConsecutiveScrollParent(this)
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (CanScrollUtils.isConsecutiveScrollParent(this) // 如果父级容器设置isConsecutive：true，则自己不消费滑动
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mScrollState != SCROLL_STATE_DRAGGING) {
                    boolean startScroll = canScrollVertically() && Math.abs(deltaY) > 0;
                    if (startScroll) {
...
//                }
        ...
        do {
            ...
            if (!isScrollBottom()) {
                ...
                if (firstVisibleView != null) {
                    awakenScrollBars();
                    int bottomOffset = CanScrollUtils.getScrollBottomOffset(firstVisibleView);
                    if (bottomOffset > 0) {
        ...
        do {
            ...
            if (!isScrollTop()) {
                ...
                if (lastVisibleView != null) {
                    awakenScrollBars();
                    int childScrollOffset = CanScrollUtils.getScrollTopOffset(lastVisibleView);
    ...
    private void scrollChild(View child, int y) {
        View scrolledView = CanScrollUtils.getScrolledView(child);
        if (scrolledView instanceof AbsListView) {
            ...
            if (scrolledView instanceof RecyclerView) {
                isInterceptRequestLayout = CanScrollUtils.startInterceptRequestLayout((RecyclerView) scrolledView);
            }
            ...
            if (isInterceptRequestLayout) {
                ...
                view.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        CanScrollUtils.stopInterceptRequestLayout(view);
                    }
                }, 0);
            }
        }
    ...
    private void checkTargetsScroll(boolean isLayoutChange, boolean isForce) {
        ...
        if (isLayoutChange) {
            while (true) {
                int bottomOffset = CanScrollUtils.getScrollBottomOffset(target);
    ...
    void scrollChildContentToTop(View target) {
        ...
        do {
            scrollY = 0;
            int offset = CanScrollUtils.getScrollTopOffset(target);
            if (offset < 0) {
                int childOldScrollY = CanScrollUtils.computeVerticalScrollOffset(target);
                scrollChild(target, offset);
                scrollY = childOldScrollY - CanScrollUtils.computeVerticalScrollOffset(target);
            }
        } while (scrollY != 0);
    }
    ...
    void scrollChildContentToBottom(View target) {
        ...
        do {
            scrollY = 0;
            int offset = CanScrollUtils.getScrollBottomOffset(target);
            if (offset > 0) {
                int childOldScrollY = CanScrollUtils.computeVerticalScrollOffset(target);
                scrollChild(target, offset);
                scrollY = childOldScrollY - CanScrollUtils.computeVerticalScrollOffset(target);
    ...
    public boolean isScrollTop() {
        ...
        if (size > 0) {
            ...
            boolean isScrollTop = getScrollY() <= 0 && !CanScrollUtils.canScrollVertically(child, -1);

            if (isScrollTop) {
                for (int i = size - 1; i >= 0; i--) {
                    View view = children.get(i);
                    if (CanScrollUtils.isConsecutiveScrollerChild(view)
                            && CanScrollUtils.canScrollVertically(view, -1)) {
                        return false;
                    }
                }
            }
    ...
    public boolean isScrollBottom() {
        ...
        if (size > 0) {
            ...
            boolean isScrollBottom = getScrollY() >= mScrollRange && !CanScrollUtils.canScrollVertically(child, 1);
    ...
    public int computeVerticalScrollRange() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!CanScrollUtils.isConsecutiveScrollerChild(child)) {
                ...
                if (CanScrollUtils.canScrollVertically(child)) {
                    View view = CanScrollUtils.getScrolledView(child);
                    range += CanScrollUtils.computeVerticalScrollRange(view) + view.getPaddingTop() + view.getPaddingBottom();
                } else {
                    range += child.getHeight();
                }
            }
    ...
    public int computeVerticalScrollOffset() {
        ...
        for (int i = 0; i < count; i++) {
            View child = children.get(i);
            if (CanScrollUtils.isConsecutiveScrollerChild(child)) {
                scrollOffset += CanScrollUtils.computeVerticalScrollOffset(child);
            }
        }
    ...
    private View getTouchTarget(int touchX, int touchY) {
        ...
        for (View touchableView : touchableViews) {
            if (CanScrollUtils.isTouchPointInView(touchableView, touchX, touchY)) {
    ...
    private boolean isIntercept(MotionEvent ev) {
        ...
        return isIntercept(CanScrollUtils.getRawX(this, ev, pointerIndex),
                CanScrollUtils.getRawY(this, ev, pointerIndex));
    }
    ...
    private boolean isIntercept(int touchX, int touchY) {
        ...
        if (target != null) {
            return CanScrollUtils.isConsecutiveScrollerChild(target);
        }
        ...
        if (scrollToIndex != -1) {
            ...
            if (offset >= 0) {
                if (getScrollY() + getPaddingTop() > scrollAnchor) {
                    ...
                    scrollOrientation = 1;
                } else if (CanScrollUtils.canScrollVertically(view, -1)) {
                    scrollOrientation = -1;
                }
            } else {
    ...
    private int getViewsScrollOffset(int index) {
        ...
        for (int i = index; i < count; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != GONE && CanScrollUtils.isConsecutiveScrollerChild(child)) {
                offset += CanScrollUtils.computeVerticalScrollOffset(child);
            }
        }
        return offset;
    }
