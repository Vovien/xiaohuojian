...
public class CanScrollUtils {
    ...
    static boolean isConsecutiveScrollerChild(View view) {
        if (view != null) {
            ...
            if (lp instanceof CanScrollConsecutiveLayout.LayoutParams) {
                return ((CanScrollConsecutiveLayout.LayoutParams) lp).isConsecutive;
            }
            return true;
        }
        return false;
    ...
    static View getScrollChild(View view) {
        if (view != null) {
            ...
            if (lp instanceof CanScrollConsecutiveLayout.LayoutParams) {
                int childId = ((CanScrollConsecutiveLayout.LayoutParams) lp).scrollChild;
    ...
    static boolean isConsecutiveScrollParent(View view) {
        View child = view;
        while (child.getParent() instanceof ViewGroup && !(child.getParent() instanceof CanScrollConsecutiveLayout)) {
            child = (View) child.getParent();
        }

        if (child.getParent() instanceof CanScrollConsecutiveLayout) {
    ...
    static boolean isTouchNotTriggerScrollStick(View rootView, int touchX, int touchY) {
        List<CanScrollConsecutiveLayout> csLayouts = getInTouchCSLayout(rootView, touchX, touchY);
        int size = csLayouts.size();
        for (int i = size - 1; i >= 0; i--) {
            CanScrollConsecutiveLayout csl = csLayouts.get(i);
            View topView = getTopViewInTouch(csl, touchX, touchY);
            if (topView != null && csl.isStickyView(topView) && csl.theChildIsStick(topView)) {
                CanScrollConsecutiveLayout.LayoutParams lp = (CanScrollConsecutiveLayout.LayoutParams) topView.getLayoutParams();
     ...
     * @return
     */
    static List<CanScrollConsecutiveLayout> getInTouchCSLayout(View rootView, int touchX, int touchY) {
        List<CanScrollConsecutiveLayout> csLayouts = new ArrayList<>();
        List<View> views = getTouchViews(rootView, touchX, touchY);
        for (View view : views) {
            if (view instanceof CanScrollConsecutiveLayout) {
                csLayouts.add((CanScrollConsecutiveLayout) view);
            }
        }
        return csLayouts;
    }

    static View getTopViewInTouch(CanScrollConsecutiveLayout csl, int touchX, int touchY) {
