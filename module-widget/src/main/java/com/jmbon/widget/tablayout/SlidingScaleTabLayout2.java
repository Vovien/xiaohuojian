...
import com.jmbon.widget.tablayout.transformer.IViewPagerTransformer;
import com.jmbon.widget.tablayout.transformer.TabScaleTransformer2;
 ...
 */
public class SlidingScaleTabLayout2 extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private TabScaleTransformer2 defaultTransformer;
    ...
    public SlidingScaleTabLayout2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingScaleTabLayout2(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param fragments 集合.
     */
    public void setViewPager(ViewPager vp, String[] titles, FragmentManager fa, ArrayList<Fragment> fragments,int defaultPosition) {
        if (vp == null) {
        ...
        this.mTitles = new ArrayList(Arrays.asList(titles));
        this.mViewPager = vp;
        this.mViewPager.setAdapter(new InnerPagerAdapter(fa, fragments, titles));
        mCurrentTab = defaultPosition;
        initViewPagerListener();
        if (defaultPosition >-1) {
            setTextSizeByPosition(defaultPosition);
        }
    ...
    private void initTransformer() {
        ...
        if (mTextUnSelectSize != mTextSelectSize) {
            defaultTransformer = new TabScaleTransformer2(this, mViewPager.getAdapter(), mTextSelectSize, mTextUnSelectSize, isDmgOpen());
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = LayoutInflater.from(mContext).inflate(R.layout.layout_scale_tab2, this, false);
    ...
    public void updateTabView(ArrayList<String> titles, int defaultPosition) {
        mTitles = titles;
        mCurrentTab = defaultPosition;
        initViewPagerListener();
        if (defaultPosition > -1) {
    ...
    private void setTextSizeByPosition(int defaultPosition) {
        for (int i = 0; i < mTabCount; i++) {
            defaultTransformer.setTextSizeByPosition(i, i == defaultPosition);
        }
    }
    ...
    public void addNewTab(String title) {
        View tabView = View.inflate(mContext, R.layout.layout_scale_tab2, null);
        if (mTitles != null) {
            mTitles.add(title);
        }
...
//        params.width = SizeUtils.dp2px(48);
//        params.width = SizeUtils.dp2px(32);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                // 被选中设置为粗体
                else tab_title.getPaint().setFakeBoldText(mTextBold == TEXT_BOLD_WHEN_SELECT && i == position);
    ...
    public void updateTabText(int index, String tabTitle) {
        if (mTitles == null || index < 0 || index >= mTitles.size()) {
            return;
        }

        mTitles.set(index, tabTitle);
        initViewPagerListener();
        setTextSizeByPosition(mCurrentTab);
    }
...
//            float textHeight = mTextPaint.descent() - mTextPaint.ascent();
    ...
    /**
     * 设置未读消息偏移,原点为文字的左上角.当控件高度固定,消息提示位置易控制,显示效果佳
     */
    public void setMsgMarginWithRight(int position, float leftPadding, float topPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        RelativeLayout tabView = (RelativeLayout) mTabsContainer.getChildAt(position);
//        tabView.setGravity(Gravity.CENTER);
        MsgView tipView = tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            ImageView title = tabView.findViewById(R.id.tv_tav_title_dmg);
            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) title.getLayoutParams();
            params.addRule(RelativeLayout.CENTER_IN_PARENT);
            ...
            lp.addRule(RelativeLayout.END_OF,title.getId());
            lp.addRule(RelativeLayout.ALIGN_TOP, title.getId());
//            lp.leftMargin = dp2px(leftPadding);
            lp.setMargins(dp2px(leftPadding), dp2px(topPadding),0, 0);
