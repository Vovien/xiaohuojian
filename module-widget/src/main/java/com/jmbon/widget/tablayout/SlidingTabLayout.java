...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    public void updateTabText(int index, String tabTitle) {
        View tabView = getTab(index);
        if (tabView == null) {
            return;
        }

        TextView tabTitleText = tabView.findViewById(R.id.tv_tab_title);
        if (tabTitleText != null) {
            tabTitleText.setText(tabTitle);
        }
    }

    public TextView getTabText(int index) {
        View tabView = getTab(index);
        if (tabView == null) {
            return null;
        }
    ...
    public void setCurrentTab(int currentTab, boolean smoothScroll) {
        if (mCurrentTab != currentTab) {
            ...
            if (mViewPager != null) {
                ...
                setCurrentTabWithNoPager(currentTab);
            }
...
//            return;
    ...
    public void showMsgWithNotMargin(int position, int num) {
        ...
        MsgView tipView = tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            UnreadMsgUtils.showNotRound2(tipView, num);
     ...
     * 设置未读消息偏移,原点为文字的右上角.当控件高度固定,消息提示位置易控制,显示效果佳
     */
    public void setMsgMarginWith(int position, float leftPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        ...
        if (tipView != null) {
            ...
            lp.addRule(RelativeLayout.ALIGN_BOTTOM, title.getId());
            tipView.setLayoutParams(lp);
        }
    }
...
//            lp.addRule(RelativeLayout.ALIGN_PARENT_END);
    ...
    public void setCurrentTabWithNoPager(int currentTab) {
