...
public class ExpandTextView extends AppCompatTextView {
    ...
    OnClickListener clickSpanListener = null;
    ...
    private final String TEXT_EXPAND = "  查看全部";
    private final String TEXT_CLOSE = "  收起全部";
    ...
    private void initCloseEnd() {
        ...
        ButtonSpan span = new ButtonSpan(getContext(), clickListener, R.color.color_currency);
        SPAN_CLOSE.setSpan(span, 0, content.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);

    }

    public void setOnSpanClickListener(@Nullable OnClickListener l) {
        clickSpanListener = l;
    }
    ...
    private Layout createWorkingLayout(String workingText) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return StaticLayout.Builder.obtain(workingText, 0, workingText.length(),
                    getPaint(), initWidth - getPaddingLeft() - getPaddingRight()).build();
        } else {
