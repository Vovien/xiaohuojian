package com.jmbon.widget.zh_scroll
...
open class BehavioralScrollView @JvmOverloads constructor(
    ...
    fun smoothScrollTo(
        scroll: Int,
        duration: Int = 300,
        onEnd: ((BehavioralScrollView) -> Unit)? = null
    ) {
...
//        super.onLayout(changed, left, top, right, bottom)
        ...
        return when (e.action) {
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                nestedScrollTarget = when (nestedScrollAxes) {
                    ViewCompat.SCROLL_AXIS_HORIZONTAL -> findHorizontalNestedScrollingTarget(
                        e.rawX,
                        e.rawY
                    )
                    ViewCompat.SCROLL_AXIS_VERTICAL -> findVerticalNestedScrollingTarget(
                        e.rawX,
                        e.rawY
                    )
                    else -> null
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        LogUtils.e("canTouchcanTouchcanTouchcanTouchcanTouch")
        if (canTouch.not()){
            return true
        }
                ...
                when (handleFirst) {
                    true -> {
                        ...
                        dispatchNestedScroll(
                            dxConsumed + selfConsumed,
                            dyConsumed,
                            dxUnconsumed - selfConsumed,
                            dyUnconsumed,
                            null,
                            type,
                            consumed
            ...
            ViewCompat.SCROLL_AXIS_VERTICAL -> {
                ...
                when (handleFirst) {
                    true -> {
                        ...
                        dispatchNestedScroll(
                            dxConsumed,
                            dyConsumed + selfConsumed,
                            dxUnconsumed,
                            dyUnconsumed - selfConsumed,
                            null,
                            type,
                            consumed
                        )
...
interface NestedScrollBehavior {
     ...
     */
    fun handleNestedPreScrollFirst(scroll: Int, @ViewCompat.NestedScrollType type: Int): Boolean? =
        null
     ...
     * @return true -> 自己优先，false -> 自己不优先，null -> 不处理 onNestedPreScroll
     */
    fun handleNestedScrollFirst(scroll: Int, @ViewCompat.NestedScrollType type: Int): Boolean? =
        false
